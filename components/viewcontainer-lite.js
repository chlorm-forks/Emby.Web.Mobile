define(['browser'],function(browser){var allPages=document.querySelectorAll('.mainAnimatedPage');var currentUrls=[];var pageContainerCount=allPages.length;var animationDuration=500;var allowAnimation=true;var selectedPageIndex=-1;function enableAnimation(){if(!allowAnimation){return false;}
...
return new Promise(function(resolve,reject){require(dependencies,function(){var animatable=allPages[pageIndex];var currentPage=animatable.querySelector('.page-view');if(currentPage){triggerDestroy(currentPage);}
var view;if(typeof(newView)=='string'){animatable.innerHTML=newView;view=animatable.querySelector('.page-view');}else{if(newViewInfo.hasScript){animatable.innerHTML='';$(newView).appendTo(animatable);}else{if(currentPage){animatable.replaceChild(newView,currentPage);}else{animatable.appendChild(newView);}}
...
function normalizeNewView(options){if(options.view.indexOf('data-role="page"')==-1){var html='<div class="page-view" data-type="'+(options.type||'')+'">';html+=options.view;html+='</div>';return html;}
...
function fade(newAnimatedPage,oldAnimatedPage,transition,isBack){var timings={duration:animationDuration,iterations:1,easing:'ease-out',fill:'both'}
...
function sendResolve(resolve,view){setTimeout(function(){resolve(view);},animationDuration);}
function getSelectedIndex(allPages){return selectedPageIndex;}
...
$.mobile=$.mobile||{};$.mobile.activePage=view;return view;});}}
...
return elem;}
function init(isAnimationAllowed){if(allowAnimation&&enableAnimation()&&!browser.animate){require(['webAnimations']);}}
return{loadView:loadView,tryRestoreView:tryRestoreView,reset:reset,setOnBeforeChange:setOnBeforeChange,init:init};});