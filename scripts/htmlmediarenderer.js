(function(){var supportsTextTracks;var hlsPlayer;var requiresSettingStartTimeOnStart;var subtitleTrackIndexToSetOnPlaying;function htmlMediaRenderer(options){var mediaElement;var self=this;function onEnded(){Events.trigger(self,'ended');}
...
function onError(e){var elem=e.target;var errorCode=elem.error?elem.error.code:'';console.log('Media element error code: '+errorCode);Events.trigger(self,'error');}
...
tracks=tracks||[];var currentTrackIndex=-1;for(var i=0,length=tracks.length;i<length;i++){if(tracks[i].isDefault){currentTrackIndex=i;break;}}
