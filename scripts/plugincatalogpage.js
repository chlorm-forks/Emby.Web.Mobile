define(['jQuery'],function($){var query={TargetSystems:'Server',IsAdult:false};function reloadList(page){Dashboard.showLoadingMsg();if(AppInfo.enableAppStorePolicy){$('.optionAdultContainer',page).hide();}else{$('.optionAdultContainer',page).show();}
...
html+="</div>";html+="</a>";html+="</div>";html+='<div class="cardFooter">';html+="<div class='cardText'>";html+=plugin.name;html+="</div>";if(!plugin.isExternal){html+="<div class='cardText' style='display:flex;align-items:center;'>";if(plugin.avgRating){html+='<iron-icon icon="star" style="color:#666;height:20px;width:20px;min-height:20px;min-width:20px;margin-right:.25em;"></iron-icon>';html+=plugin.avgRating.toFixed(1);}
